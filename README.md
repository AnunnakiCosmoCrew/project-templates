...
> **Public repos are the exception.** An org secret with visibility `all` *does* reach a
> public repo, so there the workflow is live the moment it's installed. The org has such
> an `ANTHROPIC_API_KEY`. Check before rolling the workflow out to a public repo.
> For the same reason this repo (public) doesn't carry a live copy of its own.
